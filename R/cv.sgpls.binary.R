...
{        
	...
	for ( k in 1:fold )
	{
		...
		sgpls.fit <- sgpls( train.x, train.y, K=K.val, eta=eta.val,
    ...
    if ( is.element( "multicore", installed.packages()[,1] ) ) {
        ...
        result.list <- mclapply( eta.K.list, 
            function(x) .fit.sgpls.binary( eta.val=x[1], K.val=x[2] ), 
            mc.cores = n.core )
    } else {
    ...
    if ( plot.it )
    { heatmap.spls( t(mspemat), xlab='K', ylab='eta', main='CV error Plot', coln=16, as='n' ) }
    rownames(mspemat) <- paste('eta=',eta)
    colnames(mspemat) <- paste('K =',K)
    
    cv <- list( err.mat=mspemat, eta.opt=eta.opt, K.opt=K.opt )
    return(cv)
}
