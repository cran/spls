...
\SweaveOpts{engine=R, echo=TRUE, pdf=TRUE, eps=FALSE}
...
1 and the minimum of the number of predictors and the sample size. For the yeast data, we 
search for \texttt{K} between 5 and 10 and for \texttt{eta} between 0.1 and 0.9
with the following command:

<<plot1,eval=FALSE>>=
set.seed(1)
cv.spls( yeast$x, yeast$y, eta = seq(0.1,0.9,0.1), K = c(5:10) )
@
...
by the function '\texttt{spls}'. '\texttt{print.spls}' displays
...
<<spls-fn>>=
f <- spls( yeast$x, yeast$y, eta = 0.7, K = 8 )
print(f)
coef.f <- coef(f)
coef.f[1:5,1:5]
@
...
among the selected variables. For example, 'xvar = 1' refers to  the first variable
...
<<eqtl-cv, eval=FALSE>>=
set.seed(1)
cv.spls( mice$x, mice$y, eta = seq(0.1,0.9,0.1), K = c(1:5) )
@
